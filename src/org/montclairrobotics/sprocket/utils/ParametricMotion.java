...
public class ParametricMotion extends Motion {
  ...
  public ParametricMotion(
      LinearMotion x, LinearMotion y, FallingBodyMotion z, DistanceUnit s, TimeUnit t) {
    this.x = x;
    this.y = y;
    this.z = z;
  ...
  public String positionFunctionAsString() {
    return "X: "
        + x.positionFunctionAsString()
        + ", Y: "
        + y.positionFunctionAsString()
        + ", Z: "
        + z.positionFunctionAsString();
  }
  ...
  public double positionAtTime(double t) {
    return Math.sqrt(
        Math.pow(x.positionAtTime(t), 2)
            + Math.pow(y.positionAtTime(t), 2)
            + Math.pow(z.positionAtTime(t), 2));
  }
