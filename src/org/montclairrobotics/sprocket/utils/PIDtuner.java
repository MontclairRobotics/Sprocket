...
public class PIDtuner extends PID {
  ...
  public void update() {
    ...
    if (!on) { //initialize working variables the first time around
    ...
    for (int i = lastInputs.length - 1; i >= 0; i--) {
      ...
      if (i + 1 < lastInputs.length) lastInputs[i + 1] = lastInputs[i];
    }
    lastInputs[0] = refVal;
    if (lastInputs.length
        < 9) { //we don't want to trust the maxes or mins until the inputs array has been filled
      return;
    }
    ...
    if (justchanged
        && peakCount > 2) { //we've transitioned.  check if we can autotune based on the last peaks
      double avgSeparation =
          (Math.abs(peaks[peakCount - 1] - peaks[peakCount - 2])
                  + Math.abs(peaks[peakCount - 2] - peaks[peakCount - 3]))
              / 2;
