...
/**
 * A PID controller Create with P,I,D values Call setTarget() to set the target Call get() to get
 * the correction
 *
 * <p>It just uses the math from the WPI PIDController class.
 */
public class PID implements Updatable {
  ...
  private double calculate(double val) {
    ...
    double out = P * error + I * totalError + D * (error - prevError); //+ calculateFeedForward();
