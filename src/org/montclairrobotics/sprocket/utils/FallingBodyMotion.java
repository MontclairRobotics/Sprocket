...
public class FallingBodyMotion extends Motion {
  ...
  public FallingBodyMotion(
      double initV, double initS, DistanceUnit s, TimeUnit t, double t0, double tf) {
    this.initVelocity = initV;
    this.initHeight = initS;
  ...
  public static double smallG(DistanceUnit s, TimeUnit t) {
    ...
    return DistanceUnit.m.convertQuantity(smallGInMpS2, s)
        / Math.pow(DistanceUnit.m.convertQuantity(1, t), 2);
  }
  ...
  public double positionAtTime(double t) {
    return -0.5 * FallingBodyMotion.smallG(distanceUnit, timeUnit) * Math.pow(t, 2)
        + initVelocity * t
        + initHeight;
  }
