...
public class AutoStates {
  public static class AutoAlignState extends StateObj {
    ...
    public AutoAlignState(Grip grip, DriveTrain dt, XY target, Zones zones) {
      align = new AutoAlign(grip, dt).setTarget(target).setZones(zones);
    }
  ...
  public static class DriveTime extends StateObj {
    ...
    public void update() {
      if (isDone()) dt.driveSpeedRotation(0, 0);
      else dt.driveSpeedRotation(spd, 0);
      loopsLeft--;
    }
  ...
  public static class Turn extends StateObj {
    ...
    public void update() {
      ...
      if (Math.abs(pid.getError()) < MAX_ERROR) loopsAtTarget++;
      else loopsAtTarget = 0;
    }
